...
import { getRequestsByUserId } from '@/services/requests/requestService';
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      if (currentUser?.id) {
        ...
        console.warn('No user ID available for fetching requests');
        setLoading(false);
      }
    };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="cursor-pointer transition-shadow hover:shadow-md" onClick={() => navigate('/room-reservation')}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Réservation de salle</CardTitle>
              <Building2 className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold">Réserver</div>
              <p className="text-sm text-muted-foreground">Réserver une salle pour un cours ou un événement.</p>
            </CardContent>
          </Card>

          <Card className="cursor-pointer transition-shadow hover:shadow-md" onClick={() => navigate('/equipment-request')}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Demande de matériel</CardTitle>
              <Package className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold">Demander</div>
              <p className="text-sm text-muted-foreground">Demander du matériel pour un cours ou un événement.</p>
            </CardContent>
          </Card>
        </div>
